...
protocol MapCamVCDelegate {
    func didComplete(vc: MapCamVC, lat: String, lng: String)
}
class MapCamVC: UIViewController,MGLMapViewDelegate {
    @IBOutlet var mapView: UIView!
    var mapViewVT: MGLMapView!
    public var delegate: MapCamVCDelegate?
    var style: MGLStyle!
        ...
        listCamViewModel.complete! = { [weak self] result in
            if(result == nil){
                self?.cameraList = (self?.listCamViewModel.listCamera)!
                        ...
                        // Now you can safely iterate over cameraList
                var i = 0
                        for cam in self?.cameraList ?? [] {
                            var latlng = LatLng(latitude: 0, longitude: 0)
                            latlng.latitude = cam.LAT
                            latlng.longitude = cam.LNG
                            self?.addMarkerWithLatLng(latLng: latlng)
...
//        }
    ...
    func addMarkerWithLatLng(latLng: LatLng?) {
        if let latLng = latLng, latLng.longitude > 0 {
            clearMBAnnotation()
            addMapboxMarker(coordinate: CLLocationCoordinate2D(latitude: latLng.latitude, longitude: latLng.longitude), markerIcon: UIImage(named: "icon_camera")!, identifier: String(index))
    ...
    @objc func handleSingleTap(_ recognizer: UITapGestureRecognizer) {
        ...
        addMapboxMarker(coordinate: CLLocationCoordinate2D(latitude: tapCoordinate.latitude, longitude: tapCoordinate.longitude), markerIcon: UIImage(named: "icon_camera")!, identifier: "1")
        ...
        self.style = style
            if let selectedLatLng = self.selectedLatLng {
                if selectedLatLng.latitude > 0.0 && selectedLatLng.longitude > 0.0 {
                    addMapboxMarker(coordinate:CLLocationCoordinate2D(latitude: selectedLatLng.latitude, longitude: selectedLatLng.longitude), markerIcon: UIImage(named: "icon_camera")!, identifier: "1")
                    mapViewVT.setCenter(CLLocationCoordinate2D(latitude: selectedLatLng.latitude, longitude: selectedLatLng.longitude), zoomLevel: 10, animated: true)
                } else {
                    if let userLocationCoordinate = mapViewVT.userLocation?.location?.coordinate {
                        mapViewVT.setCenter(userLocationCoordinate, zoomLevel: Zoom, animated: true)
                        getCurrentMyLocation()
